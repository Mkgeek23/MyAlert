...
<div class="row g-3 mb-4">
    <div class="col-12 col-md-4">
        <div class="card text-center h-100">
            <div class="card-body">
                <h5 class="card-title text-muted">Active Alerts</h5>
    ...
    <div class="col-12 col-md-4">
        <div class="card text-center h-100">
            <div class="card-body">
                <h5 class="card-title text-muted">Today's Alerts</h5>
    ...
    <div class="col-12 col-md-4">
        <div class="card text-center h-100">
            <div class="card-body">
                <h5 class="card-title text-muted">Overdue Alerts</h5>
...
<!-- Overdue Alerts List -->
<h2 class="mb-3">Overdue Alerts</h2>

<?php if (empty($overdueAlerts)): ?>
<div class="alert alert-success" role="alert">
    No overdue alerts. You're all caught up!
</div>
<?php else: ?>
<div class="list-group mb-4">
    ...
    <div class="list-group-item d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-2">
        <div>
            ...
            <small class="text-muted">Due: <?= htmlspecialchars($alert['next_run_at'], ENT_QUOTES, 'UTF-8') ?></small>
        </div>
        <div class="d-flex gap-2">
            <a href="<?= htmlspecialchars($baseUrl, ENT_QUOTES, 'UTF-8') ?>/alerts-edit?id=<?= (int) $alert['id'] ?>&action=close"
               class="btn btn-sm btn-warning"
               aria-label="Close alert: <?= htmlspecialchars($alert['title'], ENT_QUOTES, 'UTF-8') ?>">Close</a>
            <a href="<?= htmlspecialchars($baseUrl, ENT_QUOTES, 'UTF-8') ?>/alerts-edit?id=<?= (int) $alert['id'] ?>"
               class="btn btn-sm btn-outline-primary"
               aria-label="View alert: <?= htmlspecialchars($alert['title'], ENT_QUOTES, 'UTF-8') ?>">View</a>
        </div>
    </div>
    <?php endforeach; ?>
</div>
...
<a href="<?= htmlspecialchars($baseUrl, ENT_QUOTES, 'UTF-8') ?>/alerts-create"
   ...
   title="Create new alert"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></a>
